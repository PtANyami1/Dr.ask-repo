...
export default function App() {
  const [activeTab, setActiveTab] = useState('PhysicalExam');
                        ...
                        <AnimatePresence mode="popLayout">
                          {categoryTemplates.map((template) => (
                            <ExamCard
                              key={template.id}
                              template={template}
                              values={examState[template.id] ?? {}}
                              onChange={(key, value) => handleValueChange(template.id, key, value)}
                              onReset={() => resetExam(template.id)}
                              onShowGuide={() => setGuideInfo({ title: template.name, content: template.guideText })}
                            />
                          ))}
                        </AnimatePresence>
                      </div>
